    ...
    def _reduce_agg_metrics(self):
        ...
        if not self._metrics_to_agg:
            ...
            agg_mets = merge_dicts(
                self._metrics_to_agg, self._agg_key_funcs, self._agg_default_func
            )
...
class LoggerCollection(LightningLoggerBase):
    ...
    def name(self) -> str:
        ...
        underscore."""
        return "_".join(
            dict.fromkeys(str(logger.name) for logger in self._logger_iterable)
        )
    ...
    def version(self) -> str:
        ...
        underscore."""
        return "_".join(
            dict.fromkeys(str(logger.version) for logger in self._logger_iterable)
        )
