...
def save_hparams_to_yaml(
    config_yaml, hparams: Union[dict, Namespace], use_omegaconf: bool = True
) -> None:
    """
    Args:
    ...
    if _OMEGACONF_AVAILABLE and use_omegaconf:
        ...
        hparams = apply_to_collection(
            hparams, DictConfig, OmegaConf.to_container, resolve=True
        )
    ...
    for k, v in hparams.items():
        try:
            ...
            warn(
                f"Skipping '{k}' parameter because it is not possible to safely dump to YAML."
            )
